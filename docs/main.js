...
const preload = (tmp_idol) =>{
    let pre = []
    tmp_idol.imgs.forEach(img =>{
        let card = document.createElement('img')
        card.src = img.card
        let fullsize = document.createElement('img')
        fullsize.src = img.fullsize
        let standalone = document.createElement('img')
        standalone.src = img.standalone
        let petit = document.createElement('img')
        petit.src = img.petit
        console.log(img.title + "の画像をプリロード")
    })
}
